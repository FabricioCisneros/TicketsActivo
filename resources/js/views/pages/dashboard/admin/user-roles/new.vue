            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Añadir nuevo rol') }}</h1>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-6 shadow sm:rounded-lg">
                    ...
                    <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Informacion del rol') }}</h3>
                            <p class="mt-1 text-sm leading-5 text-gray-500">
                                {{ $t('Esta informacion sera mostrada publicamente') }}.
                            </p>
                        </div>
                        <div class="mt-5 md:mt-0 md:col-span-2">
                            <div class="grid grid-cols-3 gap-6">
                                <div class="col-span-3">
                                    <label class="block text-sm font-medium leading-5 text-gray-700" for="name">{{ $t('Nombre del rol') }}</label>
                                ...
                                <div class="col-span-3">
                                    <label class="block text-sm font-medium leading-5 text-gray-700" for="dashboard_access">{{ $t('Acceso al dashboard') }}</label>
                                    <div class="mt-1 flex rounded-md">
                                        <input-switch
                                            id="dashboard_access"
                                            v-model="userRole.dashboard_access"
                                            :disabled-label="$t('Los usuarios con este rol no tienen acceso al dashboad de la aplicacion')"
                                            :enabled-label="$t('Los usuarios con este rol pueden acceder al dashboard de la aplicacion')"
                                        ></input-switch>
                                    </div>
                                    <div class="mt-2 relative text-xs text-gray-500">
                                        {{ $t('Los usuarios con acceso al dashboard se redirigirán automáticamente al inicio del dashboard, de lo contrario, no podrán acceder al dashboard, solo tendrán acceso al servicio de ayuda') }}.
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Permisos') }}</h3>
                            <p class="mt-1 text-sm leading-5 text-gray-500">
                                {{ $t('Estas secciones de la aplicacion estan protegidas con autenticacion') }}.
                            </p>
                        </div>
                        <div class="mt-5 md:mt-0 md:col-span-2">
